...
COLORES_ACT = {
    ...
    "Trabajo autónomo": "#9467bd",
    "Sin clases (Feriado)": "#d62728",
    "Sin clases (Pausa académica)": "#8c564b",
    "Examen": "#111111",
    "Misión": "#8c564b",
}
COLORES_EVAL = {"Prueba": "#d62728", "Experimento": "#9467bd", "Trabajo práctico": "#2ca02c"}
...
def df_a_events(df: pd.DataFrame):
    events = []
    ...
    for _, r in df.iterrows():
        ...
        profesores = str(r.get("profesores", "") or "").strip()
        mision = str(r.get("misiones", "") or "").strip()
        obs = str(r.get("observaciones", "") or "").strip()

        # MISIÓN (allDay)
        if actividad == "Misión":
            ic = ICONOS_MISION.get(mision, "📌")
            title = f"{ic} {mision}" if mision else f"{ic} Misión"
            start = fecha.date().isoformat()
            end = (fecha + pd.Timedelta(days=1)).date().isoformat()
            ...
            events.append({
                ...
                "end": end,
                "allDay": True,
                "color": COLORES_ACT.get("Misión", "#888888"),
                "extendedProps": {
                    "tipo": "mision",
                    "semana": r.get("semana", ""),
                    "sección": seccion,
        ...
        # título: evaluación icono + actividad + tema
        ic_eval = ICONOS_EVAL.get(evaluacion, "")
        pref = f"{ic_eval} " if ic_eval else ""
        if tema:
            title = f"{pref}{actividad} · {tema}"
        else:
            title = f"{pref}{actividad}"

        color = COLORES_EVAL.get(evaluacion, COLORES_ACT.get(actividad, "#888888"))
...
def main():
    ...
    excel_path = config["salida"]["excel_path"]

    st.set_page_config(page_title=config["curso"]["nombre"], layout="wide")
    st.title(f"📅 {config['curso']['nombre']}")
    st.caption("Vista desde Excel generado. Para actualizar: corre `python generar_excel.py` y vuelve a desplegar.")

    if not os.path.exists(excel_path):
        st.error(f"No encuentro el Excel en: {excel_path}. Genera primero con `python generar_excel.py`.")
        st.stop()

    df = pd.read_excel(excel_path, sheet_name="Calendario")
    # normalizar tipos
    ...
    with col1:
        ...
        seccion_sel = st.multiselect("Secciones", options=secciones, default=secciones)
    with col2:
        semanas = sorted(pd.unique(df["semana"]))
        semana_sel = st.multiselect("Semanas", options=semanas, default=semanas)
    with col3:
        acts = sorted(df["actividad"].unique())
        act_sel = st.multiselect("Actividad", options=acts, default=acts)
    with col4:
        evals = sorted([x for x in df["evaluación"].fillna("").unique()])
        eval_sel = st.multiselect("Evaluación", options=evals, default=evals)

    df_f = df[
        df["sección"].isin(seccion_sel)
        & df["semana"].isin(semana_sel)
        & df["actividad"].isin(act_sel)
        & df["evaluación"].fillna("").isin(eval_sel)
    ].copy()

    tab1, tab2 = st.tabs(["Calendario", "Tabla"])

    with tab1:
        events = df_a_events(df_f)
        initial_date = pd.Timestamp(df_f["fecha"].min()).strftime("%Y-%m-%d") if len(df_f) else config["periodo"]["fecha_inicio"]
        ...
        if event_click:
            ...
            st.caption("Haz clic en un evento para ver detalles.")
